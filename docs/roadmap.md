...
5. CSS extraction: translate SCSS in `rei-cedar/src/components/<name>/styles` to `packages/cedar-ui/src/css/components/<name>.css`, keep values token-backed (no palette vars in component CSS), and derive media queries from token-generated custom media (import `packages/cedar-tokens/dist/web/breakpoints.css`).
6. Lint rules: add component rules under `tools/eslint/`, split into focused rules per constraint (like button), register them in `eslint.config.mjs`, and validate in Vue fixtures via `sandbox/vue-fixtures/eslint.config.mjs`.
7. Stories/fixtures: add/update permutations in `stories/html` and `sandbox/vue-fixtures/src/components`, render new fixtures in `sandbox/vue-fixtures/src/App.vue`, and import component CSS in `sandbox/vue-fixtures/src/main.ts`.
8. Parity checks: compare compiled CSS in `packages/cedar-ui/dist/css` with `rei-cedar/dist/style`, record accepted deltas, run `tools/parity/check-component-tokens.py` for token parity audits, and ensure all `var(--cdr-*)` references in `packages/cedar-ui/src/css` exist in `packages/cedar-tokens/dist/web` (excluding documented override custom props and `--default-outline`).
